...
public class QuanLyKhachSan_UI extends JFrame implements ActionListener, ListSelectionListener {
		...
		for (int i = 0; i < pageTrangChu.dsp.size(); i++) {
			...
			pageTrangChu.btn_ThanhToan[j].addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					dshddv = new HoaDonDV().getHDDVByMaKHAndDate(hdp.getKhachHang().getMaKH(), hdp.getNgayGioNhan(),
							hdp.getNgayGioTra());
	...
	private void handleEventTraPhong() {
		pageDatPhong.tblDatPhong.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
	...
	public void chonHDDV(HoaDonPhong hdp) {
		...
		for (int i = 0; i < dshddv.size(); i++) {
			modelHDDV.addRow(new Object[] { dshddv.get(i).getMaHDDV(), dshddv.get(i).getKhachHang().getTenKH(),
					dshddv.get(i).getNgayGioDat() });
		}
